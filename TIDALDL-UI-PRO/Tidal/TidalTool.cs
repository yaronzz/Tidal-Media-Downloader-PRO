...
namespace Tidal
{
    public class TidalTool
    {
        ...
        const int ALBUM_COVER_SIZE = 320;
        const int ARTIST_COVER_SIZE = 320;
        const int PLAYLIST_COVER_SIZE = 320;
        const int VIDEO_COVER_SIZE = 320;
        #endregion
        ...
        public static bool loginByAccesstoken(string sAccesstoken)
        {
            string sUseID = "";
            string sCountryCode = "";
            if(Session.Valid(sAccesstoken, ref sUseID, ref sCountryCode))
            {
                COUNTRY_CODE = sCountryCode;
                ACCESSTOKEN = sAccesstoken;
                Config.Countrycode(COUNTRY_CODE);
                Config.Accesstoken(ACCESSTOKEN);
                Config.Userid(sUseID);
                return true;
            }
            return false;
        }
        ...
        public static string[] getVideoDLUrls(string sID, eResolution eType, out string Errmsg, out string RetResolution)
        {
            RetResolution = "";
            ...
            RetResolution = pHash.ElementAt(iIndex).Key;
        ...
        public static string getCoverUrl(ref Album album, int size = 0)
        {
            if (size == 0)
                size = ALBUM_COVER_SIZE;
            string sRet = string.Empty;
            if (!string.IsNullOrWhiteSpace(album.Cover))
                sRet = formatCoverUrl(album.Cover, size);
        ...
        public static SearchResult Search(string sQuery, int iLimit, out string Errmsg)
        {
            ...
            for (int i = 0; i < pRet.Albums.Count; i++)
            {
                if (pRet.Albums[i].Artist == null && pRet.Albums[i].Artists != null && pRet.Albums[i].Artists.Count > 0)
                    pRet.Albums[i].Artist = pRet.Albums[i].Artists[0];
            }

            pRet.Tracks = pRet.Tracks == null ? new ObservableCollection<Track>() : pRet.Tracks;
            for (int i = 0; i < pRet.Tracks.Count; i++)
            {
                if (pRet.Tracks[i].Artist == null && pRet.Tracks[i].Artists != null && pRet.Tracks[i].Artists.Count > 0)
                    pRet.Tracks[i].Artist = pRet.Tracks[i].Artists[0];
            }

            pRet.Videos = pRet.Videos == null ? new ObservableCollection<Video>() : pRet.Videos;
            for (int i = 0; i < pRet.Videos.Count; i++)
            {
                pRet.Videos[i].DurationStr = TimeHelper.ConverIntToString(pRet.Videos[i].Duration);
                if (pRet.Videos[i].Artist == null && pRet.Videos[i].Artists != null && pRet.Videos[i].Artists.Count > 0)
                    pRet.Videos[i].Artist = pRet.Videos[i].Artists[0];
            }

            pRet.Artists = pRet.Artists == null ? new ObservableCollection<Artist>() : pRet.Artists;
            for (int i = 0; i < pRet.Artists.Count; i++)
            {
                if (pRet.Artists[i].Type == null && pRet.Artists[i].ArtistTypes != null && pRet.Artists[i].ArtistTypes.Length > 0)
                    pRet.Artists[i].Type = String.Join(";", pRet.Artists[i].ArtistTypes);
            }

            pRet.Playlists = pRet.Playlists == null ? new ObservableCollection<Playlist>() : pRet.Playlists;
            for (int i = 0; i < pRet.Playlists.Count; i++)
            {
                pRet.Playlists[i].DurationStr = TimeHelper.ConverIntToString(pRet.Playlists[i].Duration);
            }
        ...
        public static string SetMetaData(string filepath, Album TidalAlbum, Track TidalTrack, string CoverPath, ObservableCollection<Contributor> pContributors, Video TidalVideo = null)
        {
            try
            {
                ...
                if (TidalVideo != null)
                {
                    ...
                    //ReleaseDate
                    if (TidalVideo.ReleaseDate.IsNotBlank())
        ...
        {
            Dictionary<int, string> pHash = AIGS.Common.Convert.ConverEnumToDictionary(typeof(eSoundQuality), false);
            foreach (var item in pHash)
            {
        ...
        public static void setAccessToken(string sToken)
        {
            ACCESSTOKEN = sToken;
        }
        #endregion
