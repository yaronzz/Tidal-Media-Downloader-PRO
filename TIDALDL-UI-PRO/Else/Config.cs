...
namespace TIDALDL_UI.Else
{
    public class Config
    {
        ...
        public static int QualityIndex()
        {
            ...
            {
                if (sValue.ToUpper().Replace('_',' ') == pList.ElementAt(i).Value)
                    return i;
            }
            return 0;
        }
        ...
        {
            return SetOrGet("themeindex", Setvalue, "0");
        }
        ...
        {
            string sValue = Config.Resolution();
            Dictionary<int, string> pList = AIGS.Common.Convert.ConverEnumToDictionary(typeof(Tidal.eResolution));
            for (int i = 0; i < pList.Count; i++)
            {
                if(pList.ElementAt(i).Value.ToUpper().Contains(sValue.ToUpper()))
                    return pList.ElementAt(i).Key;
            }
            return 0;
        }
        ...
        public static string Key(string Setvalue = null)
        {
            return SetOrGetPrivate("key", Setvalue, "");
        }

        public static string KeyArray(string Setvalue = null)
        {
            return SetOrGetPrivate("keyarray", Setvalue, "");
        }
        #endregion
        ...
        #region Global
        private static string FFMPEG_EXIST = null;
